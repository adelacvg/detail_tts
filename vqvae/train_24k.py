...
class Trainer(object):
    def __init__(self, cfg_path):
        ...
        if self.hps.train.train_target == "gpt":
            self.G.requires_grad_(False)
            self.G.gpt.requires_grad_(True)
        elif self.hps.train.train_target == "diffusion":
            self.G.requires_grad_(False)
        ...
        with tqdm(initial = self.step, total = self.train_steps, disable = not accelerator.is_main_process) as pbar:
            while self.step < self.train_steps:
                ...
                for data in self.dataloader:
                    ...
                    wav = data['raw_wav']
                    if self.hps.train.train_target == "gpt":
                        gpt_loss = self.G(spec, spec_length,data=data,target='gpt')
                        self.G_optimizer.zero_grad()
                        self.accelerator.backward(gpt_loss)
                        grad_norm_g = commons.clip_grad_value_(self.G.parameters(), None)
                        get_grad_norm(self.G)
                        accelerator.wait_for_everyone()
                        self.G_optimizer.step()
                        accelerator.wait_for_everyone()
                        pbar.set_description(f'G_loss:{gpt_loss:.4f}')
                    elif self.hps.train.train_target == "diffusion":
                        diffusion_loss = self.G(spec, spec_length,data=data,target='diffusion')
                        self.G_optimizer.zero_grad()
                        ...
                        self.G_optimizer.step()
                        accelerator.wait_for_everyone()
                        pbar.set_description(f'G_loss:{diffusion_loss:.4f}')
                    else:
                        ...
                        unused_params.extend(list(G_.parameters()))
                        ...
                        loss_gen_all = loss_gen + loss_fm + loss_mel \
                            + loss_kl + commit_loss + diff_loss + loss_gpt + extraneous_addition
                    ...
                    if accelerator.is_main_process and self.step % self.val_freq == 0:
                        ...
                        eval_model.eval()
                        with torch.no_grad():
                            wav_eval = eval_model.infer(data['text'][0].unsqueeze(0),
                                    data['text_length'][0].unsqueeze(0), 
                                    spec[0].unsqueeze(0), 
                                    spec_length[0].unsqueeze(0))
                        eval_model.train()
                        if self.hps.train.train_target != "gpt" and self.hps.train.train_target !="diffusion":
                            scalar_dict = {
...
if __name__ == '__main__':
    ...
    trainer.load('/home/hyc/detail_tts/vqvae/logs/2024-06-26-10-33-51/model-724.pt')
    trainer.train()