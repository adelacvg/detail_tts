...
class UnifiedVoice(nn.Module):
    def __init__(self, layers=8, model_dim=512, heads=8, max_text_tokens=120, max_mel_tokens=250, max_conditioning_inputs=1,
                 mel_length_compression=2048, number_text_tokens=256,
                 start_text_token=None, number_mel_codes=8194, start_mel_token=8192,
                 stop_mel_token=8193, train_solo_embeddings=False, use_mel_codes_as_input=True,
                 checkpointing=True, types=1, spec_channels=128):
        """
        Args:
    ...
    def inference_speech(self, speech_conditioning_latent, text_inputs, mel_codes, input_tokens=None, num_return_sequences=1,
        ...
        fake_inputs = torch.full((emb.shape[0], conds.shape[1]+emb.shape[1]+mel_codes.shape[-1],), fill_value=1, dtype=torch.long,
                                 device=text_inputs.device)
